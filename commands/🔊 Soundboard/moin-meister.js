...
module.exports = {
	...
	run: async (client, message, args, cmduser, text, prefix) => {
		...
		if (!channel) {return message.reply({embeds: [new MessageEmbed().setTitle('<:no:1269533999014084683> You need to join a voice channel').setColor(es.wrongcolor).setFooter(client.getFooter(es))]});}
		...
		if (botchannel) {return message.reply({embeds: [new MessageEmbed().setTitle(`<:no:1269533999014084683> I am already connected in: \`${botchannel.name}\``).setFooter(client.getFooter(es))]});}
